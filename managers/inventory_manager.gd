...
func _on_inventory_ready( inventory ):
	...
	if inventory.has_signal( "content_changed" ) and not inventory.is_connected( "content_changed", self, "_on_inventory_content_changed" ):
...
func _on_item_picked( item, sender ):
	if has_space_for_items( [ item.get_data() ], "player" ):
		add_items( [ item ], "player" )
		sender.item_picked()
